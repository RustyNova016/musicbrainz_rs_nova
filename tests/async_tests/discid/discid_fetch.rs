...
async fn fetch_cd_id() {
    ...
    for discid in discids {
        assert!(Discid::fetch().id(discid).execute().await.is_ok(), "{} is err", discid);
    }

    assert!(Discid::fetch().id("hUbE6HKkLSkkWDvf4WliXO9OLm4-").execute().await.is_err(), "The diskid does not exist");
}
